...
Manual bank statement uploads now parse CSV, XLSX, PDF, and supported legacy XLS files server-side when `/api/statement-upload-finalize` links the uploaded Supabase Storage object, and when the legacy `/api/bank-statement-imports` route can access the raw Supabase file. The parser supports quoted CSV fields, embedded commas/newlines in quoted fields, common transaction date headers, description/payee/reference headers, signed amount columns, debit/credit columns, optional currency columns, optional running balance columns, Excel serial dates, and balance-only date/balance rows. XLSX workbooks keep the `read-excel-file` path first and select the first worksheet that can be mapped to the same row model and header semantics as CSV; legacy XLS workbooks use `@e965/xlsx` and the same normalized row model, with special handling for the inspected `account_statement` positional layout. PDF files are text-extracted with `pdf-parse`, then accepted only when they match deterministic H&W DBS current/savings, SCB current/savings, HSBC current/savings, or OSL USD/USDT extracted-text layouts. The PDF parser handles ISO, slash, and named-month dates, wrapped transaction descriptions, signed amount columns, debit/credit columns in either order, running balances, and collapsed unsigned debit/credit rows when running-balance deltas prove direction. Clearly non-bank PDFs fail with a helpful message, while unsupported bank-statement layouts remain in `pending_parse` for future parser support. Supported PDF metadata is compared to the selected Lumen entity and bank account when account holder, account name, or account number identifiers are present; absent metadata is not treated as a match and is recorded as a warning instead of blocking otherwise supported imports. Successful imports persist `statement_period_start` and `statement_period_end` from PDF statement periods when available, otherwise from the parsed transaction/balance date span, and manual imports fail before ledger upsert when that period overlaps another imported manual statement for the same selected bank account. Reprocessing the same import excludes itself from the overlap check, and duplicate detection is per bank account so multi-account uploads of the same raw PDF are not rejected globally. Corrupt PDFs, extraction/runtime errors, parser crashes, wrong selected entity/account, clearly non-bank PDFs, and overlapping already-imported periods are marked `failed`. Parsed transactions are upserted into `bank_account_transactions`; running balance and balance-only values are stored in `bank_account_balances`. Each parse/reparse attempt also writes `bank_statement_import_processing_logs` with the import, entity, bank account, trigger, start/finish timestamps, final status, parsed counts, and warning/error text.
