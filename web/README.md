...
Use a 32-byte base64 value for `XERO_TOKEN_ENCRYPTION_KEY`, for example from `openssl rand -base64 32`. The app encrypts the Xero token set with AES-GCM before writing it to `public.xero_connections`, which is locked to server-side service-role access by the migration.
