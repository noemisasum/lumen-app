...
import { daysBeforeIsoDate, formatDashboardDate, formatDashboardDateCompact, isoDateFromLocalDate, previousMonthEndIsoDate } from "../src/lib/dashboard-dates.ts";
...
assert.equal(formatDashboardDateCompact("2026-07-31"), "Jul 31, 26");
...
assert.match(dashboardSource, /formatUsdFull/);
...
assert.doesNotMatch(dashboardSource, /label: "Today"/);
assert.match(dashboardSource, /label: "Month End"/);
assert.match(dashboardSource, /dashboardDateRangeForPreset\("30d"\)/);
assert.match(dashboardSource, /Window: \{selectedDatePresetLabel\}/);
assert.match(dashboardSource, /id="date-window-menu"/);
assert.match(dashboardSource, /dateMenuRef/);
assert.match(dashboardSource, /dateMenuButtonRef/);
assert.match(dashboardSource, /dashboardWindowDays/);
assert.match(dashboardSource, /maxDashboardWindowDays = 366/);
assert.match(dashboardSource, /const \[datePreset, setDatePreset\] = useState<DatePreset>\("30d"\);/);
assert.match(dashboardSource, /const \[appliedDatePreset, setAppliedDatePreset\] = useState<DatePreset>\("30d"\);/);
...
assert.match(dashboardSource, /const \[appliedWindowDays, setAppliedWindowDays\] = useState\(defaultDateRange\.windowDays\);/);
assert.match(dashboardSource, /const appliedDateRef = useRef\(\{ asAtDate: defaultDateRange\.asAtDate, compareAsAtDate: defaultDateRange\.compareAsAtDate, windowDays: defaultDateRange\.windowDays \}\);/);
assert.match(dashboardSource, /params\.set\("windowDays", String\(nextWindowDays\)\);/);
assert.match(dashboardSource, /void loadLedgerDashboard\(session\.accessToken, appliedAsAtDate, appliedCompareAsAtDate, appliedWindowDays\);/);
assert.match(dashboardSource, /void loadLedgerDashboard\(currentSession\.accessToken, appliedDateRef\.current\.asAtDate, appliedDateRef\.current\.compareAsAtDate, appliedDateRef\.current\.windowDays\);/);
assert.match(dashboardSource, /void loadLedgerDashboard\(nextSession\.accessToken, appliedDateRef\.current\.asAtDate, appliedDateRef\.current\.compareAsAtDate, appliedDateRef\.current\.windowDays\);/);
assert.match(dashboardSource, /const nextAsAtDate = draftAsAtDate \|\| todayIsoDate\(\);/);
assert.match(dashboardSource, /void loadLedgerDashboard\(session\.accessToken, nextAsAtDate, draftCompareAsAtDate, dashboardWindowDays\(nextAsAtDate, draftCompareAsAtDate\), \{ applyDates: true, appliedPreset: "custom" \}\)/);
assert.match(dashboardSource, /draftCompareAsAtDate > effectiveAsAtDate/);
assert.match(dashboardSource, /draftCompareAsAtDate < minCompareDate/);
assert.match(dashboardSource, /min=\{daysBeforeIsoDate\(draftAsAtDate \|\| todayIsoDate\(\), maxDashboardWindowDays\)\}/);
assert.match(dashboardSource, /max=\{draftAsAtDate \|\| todayIsoDate\(\)\}/);
assert.match(dashboardSource, /function applyDatePreset\(preset: DatePreset\)/);
assert.match(dashboardSource, /appliedPreset: preset/);
assert.match(dashboardSource, /appliedPreset: "30d"/);
assert.match(dashboardSource, /fetchLedgerDashboard\(accessToken, nextCompareAsAtDate, nextWindowDays\)/);
assert.match(dashboardSource, /\.catch\(\(\) => \{/);
assert.match(dashboardSource, /function liquidityMixTotal\(rows: ExposureRow\[\]\)/);
assert.match(dashboardSource, /return absoluteMixTotal\(liquidityMixRows\(rows\)\);/);
assert.match(dashboardSource, /function liquidityMixComparison\(currentRows: ExposureRow\[\], currentConvertedCount: number, compareRows: ExposureRow\[\], compareConvertedCount: number\): LiquidityMixComparison/);
assert.match(dashboardSource, /currentConvertedCount === 0/);
assert.match(dashboardSource, /compareConvertedCount < currentConvertedCount/);
assert.match(dashboardSource, /note: "incomplete balance history"/);
assert.match(dashboardSource, /liquidityMixTotal\(currentRows\) - liquidityMixTotal\(compareRows\)/);
assert.match(dashboardSource, /const liquidityMixCompare = compareLedgerData \? liquidityMixComparison\(categoryExposure, convertedAccounts\.length, compareCategoryExposure, compareConvertedAccounts\.length\) : null;/);
assert.doesNotMatch(dashboardSource, /compareDeltaUsd/);
assert.doesNotMatch(dashboardSource, /compareTotalUsd/);
...
assert.match(dashboardSource, /Past \$\{data\.windowDays\} days/);
assert.match(dashboardSource, /month end/);
assert.match(dashboardSource, /\$\{movementContext\}: \$\{recentMovements\.length\} movements/);
...
assert.match(dashboardRouteSource, /parseAsAtDate/);
assert.match(dashboardRouteSource, /parseWindowDays/);
assert.match(dashboardRouteSource, /Use windowDays as an integer from 1 to 366\./);
assert.match(dashboardRouteSource, /windowDays,/);
assert.match(dashboardRouteSource, /parsedDate\.toISOString\(\)\.slice\(0, 10\) !== date/);
assert.match(dashboardRouteSource, /\.lte\("balance_date", dashboardDate\)/);
assert.match(dashboardRouteSource, /\.lte\("transaction_date", dashboardDate\)/);
assert.match(dashboardRouteSource, /asOf: asAtDate \? `\$\{asAtDate\}T23:59:59\.999Z` : undefined/);
assert.doesNotMatch(dashboardRouteSource, /windowDays: 30/);
...
assert.match(dashboardSource, /comparison=\{liquidityMixCompare\}/);
...
assert.doesNotMatch(dashboardSource, /label: "External Float"/);
...
assert.match(dashboardSource, /bankExposureThresholdUsd = 250000/);
assert.match(dashboardSource, /function bankRowsWithThreshold/);
assert.match(dashboardSource, /label: "Others"/);
assert.match(dashboardSource, /below \$\{formatUsdFull\(thresholdUsd\)\}/);
...
assert.match(dashboardSource, /\[\/\\buob\\b\|\\bunited\\s\+overseas\\s\+bank\\b\/i, "UOB"\]/);
assert.match(dashboardSource, /\[\/\\bdbs\\b\/i, "DBS"\]/);
...
assert.match(dashboardSource, /if \(isExcludedBankExposureAccount\(account\.accountName\)\) return false;/);
assert.match(dashboardSource, /account\.accountType === "operating_bank" \|\| account\.accountType === "client_money"/);
...
assert.match(dashboardSource, /const visibleBankExposure = useMemo\(\(\) => bankRowsWithThreshold\(bankExposure\), \[bankExposure\]\);/);
assert.match(dashboardSource, /\$\{relationshipCount\} bank\$\{relationshipCount === 1 \? "" : "s"\} below \$\{formatUsdFull\(thresholdUsd\)\}/);
assert.doesNotMatch(dashboardSource, /bank\$\{relationshipCount === 1 \? "" : "s"\} \/ \$\{accountCount\} account/);
...
assert.match(dashboardSource, /<ExposureList rows=\{visibleBankExposure\} totalUsd=\{bankTotalUsd\} emptyLabel="No USD bank balances yet\." \/>/);
