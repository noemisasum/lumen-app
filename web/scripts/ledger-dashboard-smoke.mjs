...
assert.match(dashboardSource, /void loadLedgerDashboard\(session\.accessToken, nextAsAtDate, draftCompareAsAtDate, dashboardWindowDays\(nextAsAtDate, draftCompareAsAtDate\), \{ applyDates: true \}\)/);
...
assert.match(dashboardSource, /setAppliedDashboardDates\(defaultDateRange\.asAtDate, defaultDateRange\.compareAsAtDate, defaultDateRange\.windowDays\);/);
...
assert.match(dashboardSource, /\$\{recentMovements\.length\} movements:/);
