...
assert.match(dashboardSource, /const bankExposure = useMemo\(\(\) => groupExposure\(accounts, entityNameById, "bank"\), \[accounts, entityNameById\]\);/);
assert.match(dashboardSource, /Exposure by Bank/);
assert.match(dashboardSource, /Largest banking relationships by USD balance\./);
assert.match(dashboardSource, /<ExposureList rows=\{bankExposure\} totalUsd=\{totalUsd\} emptyLabel="No USD bank balances yet\." maxRows=\{6\} \/>/);
