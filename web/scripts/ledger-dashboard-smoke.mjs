...
assert.match(dashboardSource, /LP\/MP Exposure/);
assert.match(dashboardSource, /formatPercent\(lpMpShare\)/);
assert.match(dashboardSource, /value=\{convertedAccounts\.length \? formatMoney\("USD", lpMpUsd\) : "Unavailable"\}/);
...
assert.match(dashboardSource, /<ExposureList rows={categoryExposure} totalUsd={totalUsd} emptyLabel="No USD category balances yet\." \/>/);
